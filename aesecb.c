...
int main (int argc, char* argv[])
{
    ...
    while(readlen > 0) {
        if (mode)
            AES_ecb_encrypt(filebuf, outbuf, &cryptkey, AES_DECRYPT);
        else
            AES_ecb_encrypt(filebuf, outbuf, &cryptkey, AES_ENCRYPT);

        writelen = fwrite(outbuf, 1, BUF_SIZE, outfile);
        if (writelen != BUF_SIZE) {
